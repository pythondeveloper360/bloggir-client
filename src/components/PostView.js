...
import "../App.css";
import { getPostBySlug } from "../ReqFuns";
let PostView = () => {
  let laodPost = (slug) => {
    getPostBySlug(slug)
      .then((res) => res.json())
  ...
  let [slug, setSlug] = useState(params.slug);
  let [likeNum, setLikeNum] = useState("");
  let [authorName, setAuthorName] = useState("");
  let [author_username, setAuthorUserName] = useState("");
  ...
  return (
    <div className="postPage">
      ...
      <div className="postAuthor">by {authorName || "hanzala"}</div>
      <div className="postAuthor">on {'' || "hanzala"}</div>
